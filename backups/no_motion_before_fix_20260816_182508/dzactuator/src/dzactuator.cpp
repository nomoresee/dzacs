...
turn_on_robot::turn_on_robot() : Power_voltage(0)
{
  ...
  last_velocity_command_time = ros::Time::now();
...
void turn_on_robot::Control()
{
  ...
  while (ros::ok())
  {
    ...
    // 导航或直线控制节点异常退出时，不允许持续发送最后一条非零速度。
    // 该保护及下发节拍不能依赖传感器回包；即使回包中断，也必须继续向
    // STM32 发送停车命令，避免底盘保持最后一次非零速度。
    const double command_age =
        (ros::Time::now() - last_velocity_command_time).toSec();
