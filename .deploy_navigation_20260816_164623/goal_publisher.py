...
# A 点仅作路径经过点，不参与命名。
# 物资点 -> 路径点编号；1/2 共用 B1，8/9 共用 B8，15/16 共用 B15。
...
# - B1/B3/B7/B11/B12/B15：按物资出现写入文件名
# - 文件名编号按从小到大排列，例如 11_12，而不是路径经过顺序的 12_11
# - 若物资中不含 11 且不含 12：路径仍经过 B12，但不写入文件名（静默 B12）
OPTIONAL_PATH_POINTS = {1, 3, 7, 11, 12, 15}

# 必经物资点对应路径点，不写入文件名
ALWAYS_PATH_POINTS = {4, 5, 6, 8, 10, 13, 14}
...
class GoalPublisher:
    def __init__(self):
        ...
        self.goal_subscriber_wait = rospy.get_param(
            "~goal_subscriber_wait", 10.0
        )
    ...
    def start_selected_path(self):
        ...
        if not goals:
            ...
            self.selected_path_file = None
            return
        if not self.publish_sequence_goals_with_wait(goals):
            # 发布前校验失败或节点正在退出时允许下一次裁判消息重新选路，
            # 避免 _path_started 永久锁死。
            self._path_started = False
            self.selected_path_file = None
    ...
    def publish_sequence_goals_with_wait(self, goals):
        ...
        # autostart 会并行拉起节点；必须等自动导航节点已订阅后再发送整条
        # 非锁存路径，避免第一个目标在其初始化期间丢失。
        wait_start = rospy.Time.now()
        while not rospy.is_shutdown() and self.goal_pub.get_num_connections() == 0:
            if (rospy.Time.now() - wait_start).to_sec() > self.goal_subscriber_wait:
                rospy.logwarn_throttle(
                    1.0, "等待自动导航订阅 /auto_navigation/goal (%.1fs)",
                    self.goal_subscriber_wait)
            rospy.sleep(0.05)
        ...
        with open(path, encoding="utf-8") as fp:
            for ln, line in enumerate(fp, 1):
                ...
                try:
                    ...
                    if len(parts) != 3:
                        raise ValueError("必须恰好包含 x、y、yaw 三个数")
                    ...
                    rospy.logerr(
                        "%s:%d 行格式错误，拒绝整条路线: %s", path, ln, error)
                    return []
