...
turn_on_robot::turn_on_robot() : Power_voltage(0)
{
  ...
  leftDistance = 0;
  rightDistance = 0;
  velocity_command_timeout = 0.25;
  last_velocity_command_time = ros::WallTime::now();
  ...
  private_nh.param("velocity_command_timeout",
                   velocity_command_timeout,
                   velocity_command_timeout);
  if (!std::isfinite(velocity_command_timeout) ||
      velocity_command_timeout <= 0.0)
  {
    ROS_WARN("invalid velocity_command_timeout, using 0.25 seconds");
    velocity_command_timeout = 0.25;
  }
...
void turn_on_robot::Control()
{
  ...
  while (ros::ok())
  {
    ...
    // 底盘命令使用墙钟按固定 50Hz 下发，完全不依赖传感器是否回包。
    // 导航节点失联后，watchdog 会持续发送零速命令。
    const double command_age =
        (ros::WallTime::now() - last_velocity_command_time).toSec();
    if (command_age < 0.0 || command_age > velocity_command_timeout)
    {
      if (moveBaseControl.TargetSpeed != 0)
      {
        ROS_WARN_THROTTLE(
            1.0, "velocity command timeout, forcing chassis speed to zero");
      }
    ...
    // 只有串口缓冲区已有完整帧长度时才调用旧解析函数，避免其 30ms
    // read 超时阻塞控制周期。错帧每次丢一个字节并在本周期立即继续扫描，
    // 最多扫描 256 次、处理 4 个有效帧，既能快速重同步又限制单周期工作量。
    int sensor_scan_attempts = 0;
    int valid_sensor_frames = 0;
    while (sensor_scan_attempts < 256 &&
           valid_sensor_frames < 4 &&
           Stm32_Serial.isOpen() &&
           Stm32_Serial.available() >= RECEIVE_DATA_SIZE)
    {
      ++sensor_scan_attempts;
...
void turn_on_robot::Average_filtering()
{
  ...
  const int size = static_cast<int>(imu_buff.size());
  if (size == 0)
  {
    return;
  }
...
void turn_on_robot::callback_movebase_angle(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  if (!std::isfinite(v) || !std::isfinite(w))
  {
    ...
    moveBaseControl.SpeedDirection = 0x00;
    ROS_ERROR_THROTTLE(1.0, "invalid pursuitAngle command, forcing stop");
    return;
  }
...
void turn_on_robot::callback_cmd_vel_angle(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  if (!std::isfinite(v) || !std::isfinite(w))
  {
    ...
    moveBaseControl.TargetAngle = 60;
    moveBaseControl.SpeedDirection = 0x00;
    ROS_ERROR_THROTTLE(1.0, "invalid cmd_vel command, forcing stop");
    return;
  }
