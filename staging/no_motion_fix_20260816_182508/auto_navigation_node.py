...
class AutoNavigationNode:
    ...
    def __init__(self):
        ...
        self.slope_linear_speed = rospy.get_param('~slope_linear_speed', 0.6)
        ...
        self.slope_takeover_max_anchor_age = rospy.get_param(
            '~slope_takeover_max_anchor_age', 5.0)
        self.slope_takeover_max_anchor_translation = rospy.get_param(
            '~slope_takeover_max_anchor_translation', 0.4)
        self.slope_takeover_max_anchor_rotation = rospy.get_param(
            '~slope_takeover_max_anchor_rotation', 0.5)
        self.slope_takeover_max_position_error = rospy.get_param(
            '~slope_takeover_max_position_error', 0.1)
        self.slope_takeover_max_heading_error = rospy.get_param(
            '~slope_takeover_max_heading_error', 0.35)
        # AMCL 只负责 A 点入口验收。坡段内使用高频 /odom 相对位移闭环，
        # 避免 /amcl_pose 不连续发布导致车辆在坡上误判定位失效而停车。
        self.slope_odom_timeout = rospy.get_param('~slope_odom_timeout', 0.5)
        self.slope_odom_jump_max_translation = rospy.get_param(
            '~slope_odom_jump_max_translation', 0.30)
        self.slope_odom_jump_max_rotation = rospy.get_param(
            '~slope_odom_jump_max_rotation', 0.60)
        ...
        self.slope_max_cross_track_error = rospy.get_param(
            '~slope_max_cross_track_error', 0.05)
        ...
        self.next_goal_generation = 0
        # 保存裁判系统物资点号（升序）；仅首次有效接收时写入并打印。
        self.referee_material_numbers = []
        # 保存 /amcl_pose 最近一次发布的机器人实时位姿。
        self.current_pose = None
        self.current_pose_time = None
        self.current_pose_odom_anchor = None
        # /odom 高频连续发布；坡道接管时记录 map/odom 两套入口姿态，
        # 之后只使用 odom 增量推算坡段内 map 位姿。
        self.current_odom_pose = None
        self.current_odom_time = None
        self.slope_entry_map_pose = None
        self.slope_entry_odom_pose = None
        self.current_model = ''
    ...
    def goal_callback(self, msg):
        ...
        with self.slope_control_lock:
            ...
            # 重试退避定时器存在时，新路径点只入队，不能绕过等待立即重试。
            if (self.auto_execute and not self.is_navigating and
                    self.next_goal_timer is None):
    ...
    def _execute_next_goal_locked(self):
        ...
        if self.target_found:
            ...
            return
        # 迟到的下一目标/重试定时器不能抢占正在执行的 action 目标。
    ...
    def send_move_base_goal(self, goal, done_callback, timeout, timeout_callback):
        ...
        with self.slope_control_lock:
            return self._send_move_base_goal_locked(
                goal, done_callback, timeout, timeout_callback)

    def _send_move_base_goal_locked(
            self, goal, done_callback, timeout, timeout_callback):
        self.invalidate_current_goal(stop_tracking=True)
        generation = self.goal_generation

        def done(status, result):
            # actionlib 会在其内部状态机互斥锁持有期间调用 done。不能在该
            # 回调栈内再拿坡控锁，否则会与“坡控锁内 cancel_goal”形成锁反转。
            def process_done():
                with self.slope_control_lock:
            ...
            threading.Thread(target=process_done, daemon=True).start()

        def active():
            # 这里只更新时间，不在 actionlib 内部锁持有期间获取坡控锁。
            if generation == self.goal_generation:
                self.active_callback()

        def feedback(msg):
            # 高频反馈只刷新规划时间/日志；原子状态改变仍由受锁回调完成。
            if generation == self.goal_generation:
        ...
        # 先登记超时定时器，再发送 action 目标。非常近的目标可能在
        # send_goal 返回前就完成；此顺序保证 done() 一定能关闭对应定时器。
        self.move_base_client.send_goal(goal, done, active, feedback)

    def schedule_next_goal(self, delay=0.2):
        """退出 actionlib 回调栈后再发送下一目标。"""
        with self.slope_control_lock:
            self.next_goal_generation += 1
            generation = self.next_goal_generation
            if self.next_goal_timer is not None:
                self.next_goal_timer.shutdown()

            def trigger(_event):
                with self.slope_control_lock:
                    if generation != self.next_goal_generation:
                        return
                    self.next_goal_timer = None
                    self._execute_next_goal_locked()

            self.next_goal_timer = rospy.Timer(
                rospy.Duration(delay), trigger, oneshot=True)
    ...
    def handle_planning_timeout(self):
        """处理规划超时"""
        if self.target_found or self.slope_mode_active:
            rospy.loginfo("规划超时检查已被目标返航或直线过坡状态取消")
            return
    ...
    def amcl_pose_callback(self, msg):
        ...
        if not self.pose_is_finite(msg.pose.pose):
            rospy.logwarn_throttle(1.0, "收到非有限 AMCL 位姿，已忽略")
            return
        with self.slope_control_lock:
            self.current_pose = msg.pose.pose
            # 使用消息源时间而不是回调到达时间，避免把积压的旧位姿误判为新鲜。
            self.current_pose_time = (
                msg.header.stamp if msg.header.stamp != rospy.Time()
                else rospy.Time.now())
            if (self.current_odom_pose is not None and
                    self.current_odom_time is not None and
                    abs((self.current_pose_time -
                         self.current_odom_time).to_sec()) <=
                    self.slope_odom_timeout):
                odom_position = self.current_odom_pose.position
                self.current_pose_odom_anchor = (
                    odom_position.x,
                    odom_position.y,
                    self.pose_yaw(self.current_odom_pose))
            else:
                self.current_pose_odom_anchor = None

    def odom_callback(self, msg):
        """保存高频里程计；坡段内只使用相对增量，不依赖 odom 的绝对原点。"""
        if not self.pose_is_finite(msg.pose.pose):
            rospy.logwarn_throttle(1.0, "收到非有限 /odom 位姿，已忽略")
            return
        with self.slope_control_lock:
            new_pose = msg.pose.pose
            old_pose = self.current_odom_pose
            if self.slope_mode_active and old_pose is not None:
                translation_jump = math.hypot(
                    new_pose.position.x - old_pose.position.x,
                    new_pose.position.y - old_pose.position.y)
                rotation_jump = abs(self.normalize_angle(
                    self.pose_yaw(new_pose) - self.pose_yaw(old_pose)))
                if (translation_jump > self.slope_odom_jump_max_translation or
                        rotation_jump > self.slope_odom_jump_max_rotation):
                    self.finish_slope_mode(
                        False,
                        "里程计发生跳变：平移 %.2fm、旋转 %.2frad" % (
                            translation_jump, rotation_jump))
                    return
            self.current_odom_pose = new_pose
            self.current_odom_time = (
                msg.header.stamp if msg.header.stamp != rospy.Time()
                else rospy.Time.now())
    ...
    def slope_control_pose_from_odom(self):
        """用 A 点的 map/odom 锚点和当前 odom 增量推算坡段内 map 位姿。"""
        if (self.slope_entry_map_pose is None or
                self.slope_entry_odom_pose is None):
            return None

        return self.map_pose_from_odom_anchor(
            self.slope_entry_map_pose, self.slope_entry_odom_pose)

    def slope_control_pose_from_amcl(self):
        """坡上 odom 短时失效时，使用新鲜 AMCL 位姿作为小范围回退。"""
        if self.current_pose is None or self.current_pose_time is None:
            return None
        pose_age = (rospy.Time.now() - self.current_pose_time).to_sec()
        if pose_age < 0.0 or pose_age > self.slope_odom_timeout:
            return None
        position = self.current_pose.position
        pose = (position.x, position.y, self.pose_yaw(self.current_pose))
        if not all(math.isfinite(value) for value in pose):
            return None
        return pose

    def map_pose_from_odom_anchor(self, map_anchor, odom_anchor):
        """把当前 /odom 相对锚点的位移转换到 map 坐标系。"""
        if (map_anchor is None or odom_anchor is None or
                self.current_odom_pose is None or
                self.current_odom_time is None):
            return None

        odom_age = (rospy.Time.now() - self.current_odom_time).to_sec()
        if odom_age < 0.0 or odom_age > self.slope_odom_timeout:
            return None

        entry_map_x, entry_map_y, entry_map_yaw = map_anchor
        entry_odom_x, entry_odom_y, entry_odom_yaw = odom_anchor
        odom_position = self.current_odom_pose.position
        odom_yaw = self.pose_yaw(self.current_odom_pose)

        delta_odom_x = odom_position.x - entry_odom_x
        delta_odom_y = odom_position.y - entry_odom_y
        frame_yaw = self.normalize_angle(entry_map_yaw - entry_odom_yaw)
        cos_frame = math.cos(frame_yaw)
        sin_frame = math.sin(frame_yaw)
        map_x = (entry_map_x + cos_frame * delta_odom_x -
                 sin_frame * delta_odom_y)
        map_y = (entry_map_y + sin_frame * delta_odom_x +
                 cos_frame * delta_odom_y)
        map_yaw = self.normalize_angle(
            entry_map_yaw + self.normalize_angle(odom_yaw - entry_odom_yaw))
        if not all(math.isfinite(value) for value in (map_x, map_y, map_yaw)):
            return None
    ...
    def _start_slope_mode_if_applicable_locked(self, completed_goal):
        ...
        if self.current_odom_pose is None or self.current_odom_time is None:
            rospy.logwarn("到达坡道 A 时没有 /odom；放弃直线接管，继续正常导航")
            return False
        odom_age = (rospy.Time.now() - self.current_odom_time).to_sec()
        if odom_age < 0.0 or odom_age > self.slope_odom_timeout:
            rospy.logwarn(
                "到达坡道 A 时 /odom 年龄 %.2fs 超过 %.2fs；"
                "放弃直线接管，继续正常导航",
                odom_age, self.slope_odom_timeout)
            return False
        ...
        pose_age = (rospy.Time.now() - self.current_pose_time).to_sec()
        if pose_age < 0.0:
            rospy.logwarn("到达坡道 A 时检测到 ROS 时间倒退；放弃直线接管")
            return False
        if pose_age <= self.slope_takeover_max_pose_age:
            position = self.current_pose.position
            entry_map_pose = (
                position.x, position.y, self.pose_yaw(self.current_pose))
        elif (pose_age <= self.slope_takeover_max_anchor_age and
              self.current_pose_odom_anchor is not None):
            anchor_odom_x, anchor_odom_y, anchor_odom_yaw = (
                self.current_pose_odom_anchor)
            current_odom_position = self.current_odom_pose.position
            anchor_translation = math.hypot(
                current_odom_position.x - anchor_odom_x,
                current_odom_position.y - anchor_odom_y)
            anchor_rotation = abs(self.normalize_angle(
                self.pose_yaw(self.current_odom_pose) - anchor_odom_yaw))
            if (anchor_translation >
                    self.slope_takeover_max_anchor_translation or
                    anchor_rotation > self.slope_takeover_max_anchor_rotation):
                rospy.logwarn(
                    "AMCL 锚点后的里程计变化过大：平移 %.2fm、旋转 %.2frad；"
                    "放弃直线接管",
                    anchor_translation, anchor_rotation)
                return False
            amcl_position = self.current_pose.position
            entry_map_pose = self.map_pose_from_odom_anchor(
                (amcl_position.x,
                 amcl_position.y,
                 self.pose_yaw(self.current_pose)),
                self.current_pose_odom_anchor)
            if entry_map_pose is None:
                rospy.logwarn("无法用 AMCL/odom 锚点推算 A 点位姿；放弃直线接管")
                return False
            rospy.loginfo(
                "A 点 AMCL 已 %.2fs 未刷新，使用连续 /odom 增量推算入口位姿",
                pose_age)
        else:
            rospy.logwarn(
                "到达坡道 A 时 AMCL 锚点年龄 %.2fs 超过 %.2fs；"
                "放弃直线接管，继续正常导航",
                pose_age, self.slope_takeover_max_anchor_age)
            return False

        position_x, position_y, entry_yaw = entry_map_pose
        position_error = math.hypot(
            position_x - self.slope_start[0],
            position_y - self.slope_start[1])
        ...
        odom_position = self.current_odom_pose.position
        self.slope_entry_map_pose = (position_x, position_y, entry_yaw)
        self.slope_entry_odom_pose = (
            odom_position.x,
            odom_position.y,
            self.pose_yaw(self.current_odom_pose))
        ...
        if self.slope_test_bypass_safety:
            rospy.logwarn(
                "直线过坡测试旁路已启用：激光、超时和横向偏差保护已关闭；"
                "连续里程计有效性和 B 平面硬停车仍保留")
        rospy.logwarn(
    ...
    def _slope_control_callback_locked(self):
        ...
        if (not self.slope_test_bypass_safety and
            ...
            return
        control_pose = self.slope_control_pose_from_odom()
        if control_pose is None:
            control_pose = self.slope_control_pose_from_amcl()
            if control_pose is not None:
                rospy.logwarn_throttle(
                    1.0, "坡上 /odom 暂时不可用，短时使用新鲜 AMCL 位姿")
            else:
                self.publish_slope_stop()
                rospy.logwarn_throttle(
                    1.0, "直线过坡等待有效 /odom 或 AMCL 位姿，保持停车")
                return
        position_x, position_y, current_yaw = control_pose
        command = Twist()

        # 低速对齐也会让车辆移动，所以所有运动共用同一组安全门槛。
        distance_to_end = math.hypot(
            self.slope_end[0] - position_x, self.slope_end[1] - position_y)
        ...
        rel_x = position_x - self.slope_start[0]
        rel_y = position_y - self.slope_start[1]
    ...
    def return_done_callback(self, status, result):
        ...
        with self.slope_control_lock:
            if status == actionlib.GoalStatus.SUCCEEDED:
                ...
                rospy.logerr("返航失败")
                self.target_found = False
                self.current_status = "return_failed"
                self.is_navigating = False
                self.navigation_paused = True
                self.requeue_interrupted_goal_locked("返航失败")
    ...
    def return_timeout_callback(self, event):
        ...
        with self.slope_control_lock:
            ...
            self.is_navigating = False
            self.navigation_paused = True
            self.requeue_interrupted_goal_locked("返航超时")

    def requeue_interrupted_goal_locked(self, reason):
        """返航未完成时保留被中断目标；保持暂停，等待人工确认后恢复。"""
        interrupted_goal = self.interrupted_goal
        if interrupted_goal is None:
            return
        if not any(goal is interrupted_goal for goal in self.goal_queue):
            self.goal_queue.insert(0, interrupted_goal)
        self.interrupted_goal = None
        rospy.logwarn(
            "%s；已保留被中断目标 %s，等待显式恢复导航",
            reason, interrupted_goal.description)
