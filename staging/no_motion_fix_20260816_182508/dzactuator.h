...
// 云台电机0位置限幅参数
#define GIMBAL_MOTOR0_MIN_POSITION 1000    // 云台电机0最小位置
...
public:
	...
	ros::Subscriber sub_voice_feedback;
	ros::Subscriber sub_voice_status;
	ros::Publisher pub_robot_status;
	void callback_voice_feedback(const std_msgs::String::ConstPtr &msg);
	void callback_voice_status(const std_msgs::UInt8::ConstPtr &msg);
...
public:
	...
	// 速度指令超时后底盘必须立即停车，不能持续执行最后一条非零命令。
	ros::WallTime last_velocity_command_time;
	double velocity_command_timeout;
