...
static const char *labels[OBJ_CLASS_NUM_1] = {"light"};
                   ...
                   int32_t zp, float scale)
{
  ...
  for (int a = 0; a < 3; a++)
  {
    for (int i = 0; i < grid_h; i++)
    {
      ...
      {
        int8_t box_confidence = input[(PROP_BOX_SIZE_1 * a + 4) * grid_len + i * grid_w + j];
        if (box_confidence >= thres_i8)
        {
          int offset = (PROP_BOX_SIZE_1 * a) * grid_len + i * grid_w + j;
          ...
          int maxClassId = 0;
          for (int k = 1; k < OBJ_CLASS_NUM_1; ++k)
          {
...
int post_process_1(int8_t *input0, int8_t *input1, int8_t *input2, int model_in_h, int model_in_w, float conf_threshold,
                 ...
                 std::vector<float> &qnt_scales, DetectResultsGroup *group)
{
  ...
  for (int i = 0; i < validCount; ++i)
  {
    // if (indexArray[i] == -1 || last_count >= OBJ_NUMB_MAX_SIZE_1)
    if (indexArray[i] == -1)
    {
      continue;
    }
