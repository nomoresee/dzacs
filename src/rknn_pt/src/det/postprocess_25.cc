...
static const char *labels[OBJ_CLASS_NUM] = {
    ...
    "class_20", "class_21", "class_22", "class_23", "class_24"
};
...
int post_process_25(int8_t *input0, int8_t *input1, int8_t *input2, int model_in_h, int model_in_w, float conf_threshold,
...
int draw_image_detect_25(cv::Mat &cur_img, std::vector<DetectionBox> &results, int cur_frame_id)
{
  ...
  oss << "detect_25_" << std::setfill('0') << std::setw(4) << cur_frame_id << ".jpg";
...
void show_draw_results_25(DetectResultsGroup &results_group)
{
  ...
  for (const auto& res : results_group.dets)
  {
    ...
    {
      if (res.det_name == labels[i])
      {
        class_id = i;
        break;
      }
    }
    cv::rectangle(results_group.cur_img, res.box, randColor[class_id % 25], 2, 8, 0);
