...
from std_msgs.msg import String, Bool, Int32, UInt8
...
class AutoNavigationNode:
    def __init__(self):
        ...
        self.material_position_topic = rospy.get_param('~material_position_topic', '/voice_material_position')
        ...
        self.material_position_pub = rospy.Publisher(self.material_position_topic, UInt8, queue_size=10)
    ...
    def done_callback(self, status, result):
        ...
        if status == actionlib.GoalStatus.SUCCEEDED:
            ...
            self.is_navigating = False
            self.retry_count = 0
            self.publish_material_position(self.current_goal.description)
    ...
    def extract_material_position(self, description):
        """从目标描述末尾提取点位号，例如 Road0_2 -> 2。"""
    ...
    def publish_material_position(self, description):
        position = self.extract_material_position(description)
        if position is None:
            rospy.logwarn(f"无法从目标描述提取物资点位: {description}")
            return

        msg = UInt8()
        msg.data = position
        self.material_position_pub.publish(msg)
        rospy.loginfo(f"已发布物资点位给语音模块: {position}")
